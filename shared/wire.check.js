...
function roundTrip(c) {
    ...
    if (!decoded)
        return { ok: false, bytes, decoded: null, err: 'decode returned null' };
...
function deepEqualWithTolerance(a, b) {
    ...
    if (typeof a === 'object' && typeof b === 'object' && a !== null && b !== null) {
        ...
        for (let i = 0; i < ka.length; i++) {
            if (ka[i] !== kb[i])
                return false;
            if (!deepEqualWithTolerance(a[ka[i]], b[kb[i]]))
                return false;
        }
        return true;
    }
...
if (u1 !== null) {
    console.error('  FAIL  empty buffer should decode to null');
    fail++;
}
else {
    console.log('  PASS  empty buffer → null');
    pass++;
}
const u2 = decode(new Uint8Array([0xff]));
if (u2 !== null) {
    console.error('  FAIL  unknown opcode should decode to null');
    fail++;
}
else {
    console.log('  PASS  unknown opcode → null');
    pass++;
}
