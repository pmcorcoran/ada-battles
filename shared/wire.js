...
const EVENT_BY_OP = Object.fromEntries(Object.entries(OP).map(([k, v]) => [v, k]));
const STATUS_TO_U8 = {
    lobby: 0, countdown: 1, playing: 2, ended: 3,
};
const U8_TO_STATUS = ['lobby', 'countdown', 'playing', 'ended'];
...
class Writer {
    ...
    u16(v) { this.view.setUint16(this.pos, v & 0xffff, true); this.pos += 2; }
    i16(v) { this.view.setInt16(this.pos, v, true); this.pos += 2; }
    pos16(v) { this.i16(Math.round(v * POS_SCALE)); }
    rot16(v) {
        let r = v % TWO_PI;
        if (r < 0)
            r += TWO_PI;
    ...
    view0() { return this.buf.subarray(0, this.pos); }
}
//  Reader 
class Reader {
    buf;
    ...
    pos16() { return this.i16() / POS_SCALE; }
    rot16() { return this.u16() / ROT_SCALE; }
...
const strSize = (s) => 1 + enc.encode(s).length;
...
const sizes = {
    ...
    'lobby-state': (d) => 1 + d.players.length * PLAYER_SIZE +
...
export function encode(event, data) {
    const op = OP[event];
    if (op === undefined)
        throw new Error(`wire.encode: unknown event "${event}"`);
...
export function decode(input) {
    if ((input instanceof Uint8Array ? input.length : input.byteLength) === 0)
        return null;
